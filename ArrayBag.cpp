...
**/
template<class ItemType>
   void ArrayBag<ItemType>::operator+=(const ArrayBag<ItemType>& another_array_bag_){
      for(int i = 0; i < another_array_bag_.getCurrentSize(); i++){       //previously .item_count_
         add(another_array_bag_.items_[i]);                                                
   }
}
...
**/
template<class ItemType>   
   void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& another_array_bag_){   //ArrayBag
      for(int i = 0; i < another_array_bag_.getCurrentSize(); i++){
         if(!contains(another_array_bag_.items_[i])){                                                 
            add(another_array_bag_.items_[i]);
         }
      }
}